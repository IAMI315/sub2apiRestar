...
import { playgroundNavItems, type PlaygroundNavItemKey } from '@/features/playground/navigation'
...
const playgroundIconMap: Record<PlaygroundNavItemKey, unknown> = {
  chat: ChatIcon,
  canvas: CanvasIcon
}

function buildPlaygroundNavItems(): NavItem[] {
  return playgroundNavItems.map((item) => ({
    path: item.path,
    label: t(item.labelKey),
    icon: playgroundIconMap[item.key]
  }))
}
...
//
// 条目顺序：密钥 → 聊天/画布 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换/资料。
